...
title('Step Input Response: Effect of Approximation Order', 'Interpreter', 'latex');
legend('Location', 'southeast', 'Interpreter', 'latex');

if savefigs
    uniformFigureStyle(q8_step, 'Q8_Step_Response', 18, 1/2);
end
...
title('Resonance Response: Spatial Deflection Patterns', 'Interpreter', 'latex');
legend('Location', 'northeast', 'Interpreter', 'latex');

fprintf('  Completed in %.2f sec\n', toc);

if savefigs
    uniformFigureStyle(q8_resonance, 'Q8_Resonance_Spatial', 18, 1/2);
end
...
u_zero = @(t) 0;
t_span_q8 = [0, 1];
N_values_q8_2 = [2, 5, 10];

q8_initial = figure(6); clf;
t6 = tiledlayout(length(N_values_q8_2), 1, 'Padding', 'compact', 'TileSpacing', 'compact');
...
for idx = 1:length(N_values_q8_2)
    ...
    title(sprintf('$N=%d$ modes', N_q8), 'Interpreter', 'latex');
...
if savefigs
    uniformFigureStyle(q8_initial, 'Q8_Initial_Deflection', 18, 3/4);
end

% --- Energy Conservation Check ---
fprintf('Energy Conservation Analysis (N=10):\n');
...
odefun = @(t, x) A_ss*x;
[t_sol, x_sol] = ode45(odefun, [0, 1], x0_ss);

% Compute mechanical energy
energy = zeros(length(t_sol), 1);
for i = 1:length(t_sol)
    a = x_sol(i, 1:N_q8)';
    a_dot = x_sol(i, N_q8+1:2*N_q8)';
    energy(i) = 0.5 * (sum(a_dot.^2) + sum((omega_(1:N_q8).^2) .* (a.^2)));
end

energy_variation = (max(energy) - min(energy)) / mean(energy);
fprintf('  Relative energy variation: %.2e (%.4f%%)\n', energy_variation, energy_variation*100);
